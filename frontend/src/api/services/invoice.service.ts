...
export const invoiceService = {
  ...
  async downloadPdf(id: string, filename = 'invoice.pdf'): Promise<void> {
    const { data } = await axiosClient.get<Blob>(ENDPOINTS.invoices.pdf(id), {
      responseType: 'blob',
    });
    triggerDownload(data, filename);
  },

  async downloadSharePdf(token: string, filename = 'invoice.pdf'): Promise<void> {
    const { data } = await axiosClient.get<Blob>(ENDPOINTS.invoices.publicSharePdf(token), {
      responseType: 'blob',
    });
    triggerDownload(data, filename);
  },
